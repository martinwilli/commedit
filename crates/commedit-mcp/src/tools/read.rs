...
impl CommeditServer {
    #[tool(
        description = "List the commits of the checked-out branch (the ancestors of HEAD, newest first, like `git log`), with their branch/tag decorations. Returns up to `limit` commits (default 30) from `offset`; when `has_more`, page on with the returned `next_offset`. Each commit always carries sha, change_id and subject; `is_merge` and `refs` appear only when set (a merge, a decorated commit) and are otherwise omitted (absent means a non-merge / no decoration). By default it is a lean header-only overview; pass `fields` to add the verbose fields you need (message, identity, parents — e.g. just the timestamps when re-dating), then show_commit for any one commit's full message and diff. Merge commits are included but cannot be moved, dropped, split or used as a squash source. Shas/change_ids are abbreviated to the shortest repo-unique prefix (>= 8 chars) — pass them straight back as a commit ref; shas change on every mutation while the change_id is stable, so prefer change ids over re-listing. Set `working_copy: true` to also get the uncommitted-changes status inline (same as working_copy_status), saving a round-trip."
    )]
