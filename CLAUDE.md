...
- Releases are cut by pushing a signed tag that points at a version-bump commit, which also publishes the plugin marketplace. The full procedure, the one-time repository setup it depends on, and how to verify the result are in [`RELEASING.md`](RELEASING.md).
- When integrating a feature branch into `master`, force a merge commit (`git merge --no-ff`) and give it a body in the same command: keep git's `Merge branch 'feature/…'` subject, then one or two sentences on what it introduces (`-m "Merge branch '…'" -m "…"`). Wrap that body yourself at ~72 columns: git never reflows `-m` text, and a second `-m` only starts a new paragraph. For more than one line, prefer `git merge --no-ff -F -` with a heredoc. No separate reword step — commedit is for rewriting existing history and rebasing descendants, which a fresh merge at HEAD doesn't need.
